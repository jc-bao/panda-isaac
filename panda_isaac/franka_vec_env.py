...
from panda_isaac.vec_env import GymVecEnv
...
class GymFrankaVecEnv(GymVecEnv):
  ...
  def _setup_single_env_gen(self, cfg):
    """get function to setup env and env.actors
          Args:
                  cfg (dict): dict file for actor
          Returns:
    ...
    self._actuation_mode = self._ACTUATION_MODE_MAP[cfg['franka']
                                                    ['action']['mode']]
    franka = GymFranka(cfg['franka'], self._scene,
                       actuation_mode=self._actuation_mode)
    self._frankas = [franka] * self.n_envs

    table_transform = gymapi.Transform(p=gymapi.Vec3(
      cfg['table']['dims']['sx']/3, 0, cfg['table']['dims']['sz']/2))
    self._franka_transform = gymapi.Transform(
    ...
    def setup(scene, _):
      ...
      scene.add_asset(self._franka_name, franka,
                      self._franka_transform, collision_filter=1)
    return setup
  ...
  def _init_action_space(self, cfg):
    ...
    if self._action_mode == 'vic':
      ...
      self._init_ee_transforms = [
        self._frankas[env_idx].get_desired_ee_transform(
          env_idx, self._franka_name)
  ...
  def _apply_actions(self, all_actions):
    for env_idx in self._scene.env_idxs:
      ...
      if self._action_mode == 'vic':
        ...
        self._frankas[env_idx].set_delta_ee_transform(
          env_idx, self._franka_name, delta_transform)
      elif self._action_mode == 'hfpc' or self._action_mode == 'hfpc_cartesian_gains':
        xa_tf = self._frankas[env_idx].get_ee_transform(
          env_idx, self._franka_name)
        ...
        J = self._frankas[env_idx].get_jacobian(
          env_idx, self._franka_name)

        # The last two points are finger joints.
        qdot = self._frankas[env_idx].get_joints_velocity(
          env_idx, self._franka_name)[:7]
        xdot = np.matmul(J, qdot)

        xd_position = xa[:3] + arm_action[:3]
        xd_orient_rpy = arm_action[3:6]
        xd_orient = gymapi.Quat.from_euler_zyx(xd_orient_rpy)
        ...
        self._frankas[env_idx].apply_torque(
          env_idx, self._franka_name, tau)
      elif self._action_mode == 'joints':
        # add dummy gripper joint cmds
        delta_joints = np.concatenate([arm_action, [0, 0]])
        self._frankas[env_idx].apply_delta_joint_targets(
          env_idx, self._franka_name, delta_joints)
      else:
        raise ValueError(f"Invalid action mode: {self._action_mode}")

      gripper_action = action[-1]
      gripper_width = np.clip(gripper_action, 0, 0.04)
      self._frankas[env_idx].set_gripper_width_target(
  ...
  def _compute_obs(self, all_actions):
    ...
    for env_idx in self._scene.env_idxs:
      all_joints = self._frankas[env_idx].get_joints(
        env_idx, self._franka_name)
      ee_transform = self._frankas[env_idx].get_ee_transform(
        env_idx, self._franka_name)
      ee_ct_forces = self._frankas[env_idx].get_ee_ct_forces(
        env_idx, self._franka_name)

      all_obs[env_idx, :7] = all_joints[:7]
      # gripper width is 2 * each gripper's prismatic length
      all_obs[env_idx, 7] = all_joints[-1] * 2
      all_obs[env_idx, 8:15] = transform_to_np(
        ee_transform, format='wxyz')
  ...
  def _reset(self, env_idxs):
    if not self._has_first_reset:
      ...
      for env_idx in env_idxs:
        self._init_joints.append(
          self._frankas[env_idx].get_joints(env_idx, self._franka_name))

    for env_idx in env_idxs:
      self._frankas[env_idx].set_joints(
        env_idx, self._franka_name, self._init_joints[env_idx])
      self._frankas[env_idx].set_joints_targets(
        env_idx, self._franka_name, self._init_joints[env_idx])
      ...
      if self._action_mode == 'joints':
        if 'randomize_joints' in self._cfg['franka'] and self._cfg['franka']['randomize_joints']:
          ...
          self._frankas[env_idx].set_joints(
            env_idx, self._franka_name, init_random_joints)
          self._frankas[env_idx].set_joints_targets(
            env_idx, self._franka_name, init_random_joints)
...
class GymFrankaBlockVecEnv(GymFrankaVecEnv):
    ...
    for env_idx in env_idxs:
      block_pose = gymapi.Transform(
        p=gymapi.Vec3(
          ...
          np.random.rand() * 0.2,
          self._cfg['table']['dims']['sz'] +
          self._cfg['block']['dims']['sz'] / 2 + 0.05
        ))
  ...
  def _compute_obs(self, all_actions):
    ...
    for env_idx in self._scene.env_idxs:
      block_transform = self._block.get_rb_transforms(
        env_idx, self._block_name)[0]
      box_pose_obs[env_idx, :] = transform_to_np(
        block_transform, format='wxyz')
