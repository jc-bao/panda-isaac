...
class GymScene:
  ...
  def setup_all_envs(self, setup):
    ...
    if self.use_gpu_pipeline:
      ...
      self.gym.prepare_sim(self.sim) # to use tensor API
      self._tensors = {
    ...
    if self.physics_engine == gymapi.SIM_PHYSX and self.use_gpu_pipeline:
      ...
      #   (env_index, link_index, axis_index, dof_index + 6) // not fix base
      #   (num_envs, num_links - 1, 6, num_dofs) // fix base
      # get the index from jacobian:
      ...
      # jacobian[env_index, link_index, :, dof_index + 6] // not fix base
      # jacobian[env_index, link_index - 1, :, dof_index] // fix base
  ...
  def add_asset(self, name, asset, poses, collision_filter=0):
    """load all agents (blocks, robots etc). create all actors

    Args:
        name (_type_): _description_
        asset (_type_): _description_
        poses (_type_): _description_
        collision_filter (int, optional): _description_. Defaults to 0.
  ...
  def step(self):
    ...
    self.gym.simulate(self.sim) # key method
    self.gym.fetch_results(self.sim, True)

    if self.use_gpu_pipeline:
      self.gym.refresh_actor_root_state_tensor(self.sim) # update all base tensors
      self.gym.refresh_net_contact_force_tensor(self.sim)
      self.gym.refresh_dof_state_tensor(self.sim) # update dof positions
  ...
  def render_cameras(self):
    self.gym.step_graphics(self.sim)
    self.gym.render_all_camera_sensors(self.sim) # key method
...
def make_gym(sim_cfg):
  ...
  """Create Simulation
	Args:
			compute_device: 'cuda:0' for sim
			graphics_device: 'cuda:0' for render
