...
def change_basis(T, R_cb, left=True, right=True):
  if left:
    T = RigidTransform(
      rotation=R_cb.T, from_frame=T.to_frame, to_frame=T.to_frame) * T
  if right:
    T = T * RigidTransform(rotation=R_cb,
...
def real_to_sim_franka_transform(Treal_frame_to_base, Tsim_base_to_world=None):
  Tsim_frame_to_base = change_basis(
    Treal_frame_to_base, _R_sim_to_real_franka, left=True, right=False)
  if Tsim_base_to_world is None:
    Tsim_frame_to_world = Tsim_frame_to_base.as_frames(
      Tsim_frame_to_base.from_frame, 'world')
  else:
...
def sim_to_real_franka_transform(Tsim_frame_to_world, Tsim_base_to_world=None):
  ...
  Treal_frame_to_base = change_basis(
    Tsim_frame_to_base, _R_sim_to_real_franka.T, left=True, right=False)
  Treal_frame_to_world = Treal_frame_to_base.as_frames(
    Treal_frame_to_base.from_frame, 'world')
...
def compute_task_space_impedance_control(J, curr_transform, target_transform, x_vel, Ks, Ds):
  ...
  x_quat = quaternion.from_float_array(
    quat_to_np(curr_transform.r, format='wxyz'))

  xd_pos = vec3_to_np(target_transform.p)
  xd_quat = quaternion.from_float_array(
    quat_to_np(target_transform.r, format='wxyz'))
